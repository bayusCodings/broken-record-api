...
### Docker for Redis Emulator
To use the Redis Emulator, you can start it using Docker:
```
npm run redis:start
```
This will start a Redis instance running on your local machine. You can customize the settings in the redis setup by modifying the docker-compose-redis.yml if necessary. In the current configuration, you will have a Redis container running, which is accessible at localhost:6379.
This redis uri will be necessary on the .env file, with example as follows:

```
REDIS_URI=redis://localhost:6379
```
This will point your application to a local redis instance.
...
http://localhost:3000
```
Link to the Broken Record Admin Page
